...
**真实模型模式**（deepseek-v4-flash，单轮迭代容器内 harness 实测，golden set=4）：
...
引用准确率     1.00
证据充分性     1.00
任务成功率     0.25     # 单轮下 Critic 打回未生效（迭代上限强制放行），多轮收敛后覆盖会更完整
judge 一致性   kappa 0.0
```
...
说明：真实模式跑分受 `ATHENA_MAX_ITERATIONS` 配置影响显著——单轮（演示容器默认）Critic 打回回路不生效，任务覆盖率偏低；调高迭代上限（如 CI 基线 3 轮）后 Critic 会带反馈重检，覆盖率随之上升。`judge_expert_kappa` 在小样本（4 用例）下统计意义有限，扩充 golden set 后更有参考价值。