...
四维评测也跑过真实全量：3 轮迭代下综合分 0.8775、覆盖率 0.92、引用准确率 100%；对比单轮的 0.80 和 0.67——同一系统只改迭代上限的对照跑分，直接量化了 Critic 打回回路的价值。

这套系统让我能清楚知道每个 Agent 花了多少时间、多少钱，以及每个质量机制带来了多少提升，而不是一个黑盒。"
